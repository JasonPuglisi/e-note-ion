...
def _kmeans_dominant(pixels: list[tuple[float, float, float]]) -> tuple[float, float, float]:
  ...
  first = random.choice(pixels)  # nosec B311  # k-means++ centroid init, not a security context
  ...
  for _ in range(k - 1):
    ...
    if total == 0:
      break
    threshold = random.random() * total  # nosec B311  # weighted distance sampling, not a security context
    cumulative = 0.0
