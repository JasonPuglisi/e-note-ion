...
def test_flat_message_credentials_are_neither_migrated_nor_accepted(
  tmp_path: Any, monkeypatch: pytest.MonkeyPatch
) -> None:
  """2.0 removed the auto-migration (#431), and did not replace it with a check.

  A flat credential scoped to message is left alone on disk and simply does not
  authenticate. Carrying detection code for a format we no longer support is
  the cruft a major version exists to delete; the migration lives in the
  ...
  monkeypatch.setattr(
    ...
    '_config',
    {
      'webhook': {
        'port': 8080,
        'credentials': {'alice': {'secret_hash': '$argon2id$alice', 'webhooks': ['message']}},
      }
    },
  )
  ...
  # Untouched on disk — no silent rewrite of the user's file.
  text = config_file.read_text()
  assert '[webhook.credentials.alice]' in text
  assert '[webhook.credentials.message.friend.alice]' not in text
  # And inert.
  assert 'alice' not in _config_mod.get_credentials('message')
...
def test_named_credential_authenticates_scoped_endpoint() -> None:
  ...
  with patch.dict(
    'config._config',
    # Nested namespace: 2.0 removed flat message credentials (#431).
    {'webhook': {'credentials': {'message': {'friend': {'alice': {'secret_hash': friend_hash}}}}}},
  ):
    with patch.object(_mod, '_get_integration', return_value=mock_mod):
      server, port = _start_test_server()
      try:
