...
| `public` | `false` | When `true`, skip templates marked `private = true` (for shared/guest-visible spaces). Can be toggled at runtime via `POST /webhook/scheduler` with `{"action": "set_public", "value": true/false}` |
